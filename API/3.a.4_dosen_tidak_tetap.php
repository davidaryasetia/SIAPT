...
/* 
 Tabel 3.a.4) Jabatan Akademik Tidak Dosen Tetap 
 Dosen Tidak (Honorer/PPNPN, Tenaga Kontrak)
 ...
 - STATUS_KARYAWAN.NOMOR 3=Honorer/PPNPN, 6=Tenaga Kontrak
*/ 
...
CASE PEGAWAI_PENDIDIKAN_TINGKAT.NOMOR
WHEN 10 THEN \'Doktor/Doktor Terapan\'
WHEN 11 THEN \'Doktor/Doktor Terapan(Guru Besar)\'
...
COUNT(DISTINCT CASE WHEN STAFF.NOMOR IN (4,8,26,27,59)
THEN PEGAWAI.NIP END) AS "Tenaga Pengajar"

/* Error :(
SUM (
COUNT(DISTINCT CASE WHEN JABATAN_FUNGSIONAL.NOMOR=5 THEN PEGAWAI.NIP END) + 
COUNT(DISTINCT CASE WHEN JABATAN_FUNGSIONAL.NOMOR=3 THEN PEGAWAI.NIP END) +  
COUNT(DISTINCT CASE WHEN JABATAN_FUNGSIONAL.NOMOR=2 THEN PEGAWAI.NIP END) +  
COUNT(DISTINCT CASE WHEN JABATAN_FUNGSIONAL.NOMOR=1 THEN PEGAWAI.NIP END) +
COUNT(DISTINCT CASE WHEN STAFF.NOMOR IN (4,8,26,57,59)
THEN PEGAWAI.NIP END)
) AS "Jumlah Total"
*/
...
WHERE STATUS_PEGAWAI.NOMOR IN (1,6,7,8,10,11,12) AND  	 
 STATUS_KARYAWAN.NOMOR IN (3,6) AND 
 PEGAWAI_PENDIDIKAN_TINGKAT.NOMOR IN (10,11,12) AND 
 STAFF.NOMOR IN (4,8,26,57,59)
GROUP BY PEGAWAI_PENDIDIKAN_TINGKAT.NOMOR, PEGAWAI_PENDIDIKAN_TINGKAT.PENDIDIKAN, STAFF.STAFF
ORDER BY 1 ASC';
