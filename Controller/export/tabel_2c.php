...
foreach ($sks as $row) {
    ...
    $activeWorksheet->getStyle('C2:G' . ($rowIndex - 1))->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
...
$teori =     '=SUM(C5:C' . ($rowIndex - 1) . ')';
$praktikum = '=SUM(D5:D' . ($rowIndex - 1) . ')';
$praktik =   '=SUM(E5:E' . ($rowIndex - 1) . ')';
$pkl =       '=SUM(F5:F' . ($rowIndex - 1) . ')';
$total =     '=SUM(G5:G' . ($rowIndex - 1) . ')';
